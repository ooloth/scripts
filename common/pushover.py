...
def send_notification(*, title: str, html: str, dry_run: bool = False) -> None:
    ...
    try:
        ...
        client.send_message(
            user=get_secret(OP_ITEM, OP_FIELD_USER_KEY), title=title, message=html, html=True
        )
        log.info("✅ Notification sent successfully.")
    except Exception:
        log.error(f"🚨 There was a problem sending the '{title}' notification.", exc_info=True)
